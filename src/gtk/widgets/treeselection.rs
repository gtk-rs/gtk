...
impl TreeSelection {
    pub fn set_mode(&self, mode: gtk::SelectionMode) {
        unsafe { ffi::gtk_tree_selection_set_mode(GTK_TREE_SELECTION(self.pointer), mode) }
    }

    pub fn get_mode(&self) -> gtk::SelectionMode{
        unsafe { ffi::gtk_tree_selection_get_mode(GTK_TREE_SELECTION(self.pointer)) }
    }

    pub fn get_user_data<'r, T>(&mut self) -> &'r mut T {
        unsafe { ::std::mem::transmute(ffi::gtk_tree_selection_get_user_data(GTK_TREE_SELECTION(self.pointer))) }
    }
    ...
    pub fn get_selected(&self, model: &gtk::TreeModel, iter: &gtk::TreeIter) -> bool {
        match unsafe { ffi::gtk_tree_selection_get_selected(GTK_TREE_SELECTION(self.pointer), &mut model.get_pointer(),
            iter.get_pointer()) } {
            0 => false,
            _ => true
        }
    ...
    pub fn path_is_selected(&self, path: &TreePath) -> bool {
        match unsafe { ffi::gtk_tree_selection_path_is_selected(GTK_TREE_SELECTION(self.pointer), path.get_pointer()) } {
            0 => false,
            _ => true
        }
    }
    ...
    pub fn iter_is_selected(&self, iter: &TreeIter) -> bool {
        match unsafe { ffi::gtk_tree_selection_iter_is_selected(GTK_TREE_SELECTION(self.pointer), iter.get_pointer()) } {
            0 => false,
            _ => true
        }
    }
    ...
    pub fn select_all(&self) {
        unsafe { ffi::gtk_tree_selection_select_all(GTK_TREE_SELECTION(self.pointer)) }
    }

    pub fn unselect_all(&self) {
        unsafe { ffi::gtk_tree_selection_unselect_all(GTK_TREE_SELECTION(self.pointer)) }
    }

    pub fn select_range(&self, start_path: &TreePath, end_path: &TreePath) {
        unsafe { ffi::gtk_tree_selection_select_range(GTK_TREE_SELECTION(self.pointer), start_path.get_pointer(),
            end_path.get_pointer()) }
    }

    pub fn unselect_range(&self, start_path: &TreePath, end_path: &TreePath) {
        unsafe { ffi::gtk_tree_selection_unselect_range(GTK_TREE_SELECTION(self.pointer), start_path.get_pointer(),
            end_path.get_pointer()) }
    }
}

impl_drop!(TreeSelection)
impl_TraitWidget!(TreeSelection)