...
pub trait CellAreaExt {
    fn activate<T: Upcast<Widget>>(&self, context: &CellAreaContext, widget: &T, cell_area: &Rectangle, flags: CellRendererState, edit_only: bool) -> bool;
    //fn activate_cell<T: Upcast<Widget>, U: Upcast<CellRenderer>>(&self, widget: &T, renderer: &U, event: /*Unknown conversion*/Unknown rust type: "Event", cell_area: &Rectangle, flags: CellRendererState) -> bool;
    ...
    #[cfg(gtk_3_14)]
    fn attribute_get_column<T: Upcast<CellRenderer>>(&self, renderer: &T, attribute: &str) -> i32;
    //fn cell_get<T: Upcast<CellRenderer>>(&self, renderer: &T, first_prop_name: &str, : /*Unknown conversion*/Fundamental: VarArgs);
    //fn cell_get_property<T: Upcast<CellRenderer>>(&self, renderer: &T, property_name: &str, value: /*Ignored*/&mut gobject::Value);
    //fn cell_get_valist<T: Upcast<CellRenderer>>(&self, renderer: &T, first_property_name: &str, var_args: /*Unknown conversion*/Unsupported);
    //fn cell_set<T: Upcast<CellRenderer>>(&self, renderer: &T, first_prop_name: &str, : /*Unknown conversion*/Fundamental: VarArgs);
    //fn cell_set_property<T: Upcast<CellRenderer>>(&self, renderer: &T, property_name: &str, value: /*Ignored*/&gobject::Value);
    //fn cell_set_valist<T: Upcast<CellRenderer>>(&self, renderer: &T, first_property_name: &str, var_args: /*Unknown conversion*/Unsupported);
    fn copy_context(&self, context: &CellAreaContext) -> Option<CellAreaContext>;
    fn create_context(&self) -> Option<CellAreaContext>;
    //fn event<T: Upcast<Widget>>(&self, context: &CellAreaContext, widget: &T, event: /*Unknown conversion*/Unknown rust type: "Event", cell_area: &Rectangle, flags: CellRendererState) -> i32;
    ...
    fn get_preferred_width_for_height<T: Upcast<Widget>>(&self, context: &CellAreaContext, widget: &T, height: i32) -> (i32, i32);
    fn get_request_mode(&self) -> SizeRequestMode;
    fn has_renderer<T: Upcast<CellRenderer>>(&self, renderer: &T) -> bool;
    fn inner_cell_area<T: Upcast<Widget>>(&self, widget: &T, cell_area: &Rectangle) -> Rectangle;
    fn is_activatable(&self) -> bool;
    fn is_focus_sibling<T: Upcast<CellRenderer>, U: Upcast<CellRenderer>>(&self, renderer: &T, sibling: &U) -> bool;
    fn remove<T: Upcast<CellRenderer>>(&self, renderer: &T);
    fn remove_focus_sibling<T: Upcast<CellRenderer>, U: Upcast<CellRenderer>>(&self, renderer: &T, sibling: &U);
    //fn render<T: Upcast<Widget>>(&self, context: &CellAreaContext, widget: &T, cr: /*Ignored*/&mut cairo::Context, background_area: &Rectangle, cell_area: &Rectangle, flags: CellRendererState, paint_focus: bool);
    fn request_renderer<T: Upcast<CellRenderer>, U: Upcast<Widget>>(&self, renderer: &T, orientation: Orientation, widget: &U, for_size: i32) -> (i32, i32);
    fn set_focus_cell<T: Upcast<CellRenderer>>(&self, renderer: &T);
    fn stop_editing(&self, canceled: bool);
}

impl<O: Upcast<CellArea>> CellAreaExt for O {
    fn activate<T: Upcast<Widget>>(&self, context: &CellAreaContext, widget: &T, cell_area: &Rectangle, flags: CellRendererState, edit_only: bool) -> bool {
        unsafe {
