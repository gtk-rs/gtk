...
pub trait CellAreaExt {
    ...
    fn connect_focus_changed<F: Fn(&Self, &CellRenderer, &str) + 'static>(&self, f: F) -> u64;
...
impl<O: IsA<CellArea> + IsA<Object>> CellAreaExt for O {
    ...
    fn connect_focus_changed<F: Fn(&Self, &CellRenderer, &str) + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&Self, &CellRenderer, &str) + 'static>> = Box_::new(Box_::new(f));
...
unsafe extern "C" fn apply_attributes_trampoline<T>(this: *mut GtkCellArea, model: *mut GtkTreeModel, iter: *mut GtkTreeIter, is_expander: gboolean, is_expanded: gboolean, f: gpointer)
where T: IsA<CellArea> {
    callback_guard!();
    let f: &Box_<Fn(&T, &TreeModel, &TreeIter, bool, bool) + 'static> = transmute(f);
    f(&CellArea::from_glib_none(this).downcast_unchecked(), &from_glib_none(model), &from_glib_none(iter), from_glib(is_expander), from_glib(is_expanded))
}

unsafe extern "C" fn focus_changed_trampoline<T>(this: *mut GtkCellArea, renderer: *mut GtkCellRenderer, path: *mut c_char, f: gpointer)
where T: IsA<CellArea> {
    callback_guard!();
    let f: &Box_<Fn(&T, &CellRenderer, &str) + 'static> = transmute(f);
    f(&CellArea::from_glib_none(this).downcast_unchecked(), &from_glib_none(renderer), &String::from_glib_none(path))
}

unsafe extern "C" fn remove_editable_trampoline<T>(this: *mut GtkCellArea, renderer: *mut GtkCellRenderer, editable: *mut GtkCellEditable, f: gpointer)
where T: IsA<CellArea> {
    callback_guard!();
