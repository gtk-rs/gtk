...
impl SpinButton {
    ...
    pub fn connect_change_value<F: Fn(&SpinButton, ScrollType) + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&SpinButton, ScrollType) + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "change-value",
                transmute(change_value_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }

    //pub fn connect_input<Unsupported or ignored types>(&self, f: F) -> u64 {
    //    TODO: connect to trampoline
    //}

    pub fn connect_output<F: Fn(&SpinButton) -> bool + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&SpinButton) -> bool + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "output",
                transmute(output_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }

    pub fn connect_value_changed<F: Fn(&SpinButton) + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&SpinButton) + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "value-changed",
                transmute(value_changed_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }

    pub fn connect_wrapped<F: Fn(&SpinButton) + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&SpinButton) + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "wrapped",
                transmute(wrapped_trampoline as usize), Box_::into_raw(f) as *mut _)
        }
    }
}

unsafe extern "C" fn change_value_trampoline(this: *mut GtkSpinButton, scroll: GtkScrollType, f: gpointer) {
    callback_guard!();
    let f: &Box_<Fn(&SpinButton, ScrollType) + 'static> = transmute(f);
    f(&from_glib_none(this), from_glib(scroll))
}

unsafe extern "C" fn output_trampoline(this: *mut GtkSpinButton, f: gpointer) -> gboolean {
    callback_guard!();
    let f: &Box_<Fn(&SpinButton) -> bool + 'static> = transmute(f);
    f(&from_glib_none(this)).to_glib()
}

unsafe extern "C" fn value_changed_trampoline(this: *mut GtkSpinButton, f: gpointer) {
    callback_guard!();
    let f: &Box_<Fn(&SpinButton) + 'static> = transmute(f);
    f(&from_glib_none(this))
}

unsafe extern "C" fn wrapped_trampoline(this: *mut GtkSpinButton, f: gpointer) {
    callback_guard!();
    let f: &Box_<Fn(&SpinButton) + 'static> = transmute(f);
    f(&from_glib_none(this))
}
