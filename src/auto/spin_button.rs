...
impl SpinButton {
    pub fn new<'a, P: Into<Option<&'a Adjustment>>>(adjustment: P, climb_rate: f64, digits: u32) -> SpinButton {
        ...
        unsafe {
            Widget::from_glib_none(ffi::gtk_spin_button_new(adjustment, climb_rate, digits)).downcast_unchecked()
        }
    }
    ...
    pub fn configure<'a, P: Into<Option<&'a Adjustment>>>(&self, adjustment: P, climb_rate: f64, digits: u32) {
        let adjustment = adjustment.into();
        let adjustment = adjustment.to_glib_none().0;
        unsafe {
            ffi::gtk_spin_button_configure(self.to_glib_none().0, adjustment, climb_rate, digits);
        }
    }
