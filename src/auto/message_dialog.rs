...
pub trait MessageDialogExt {
    ...
    fn connect_property_buttons_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_image_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_message_area_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_message_type_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_secondary_text_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_secondary_use_markup_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;
...
where P: IsA<MessageDialog> {
    ...
    f(&MessageDialog::from_glib_none(this).downcast_unchecked())
}
