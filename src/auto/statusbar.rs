...
unsafe extern "C" fn text_popped_trampoline(this: *mut GtkStatusbar, context_id: c_uint, text: *mut c_char, f: gpointer) {
    callback_guard!();
    let f: &Box_<Fn(&Statusbar, u32, &str) + 'static> = transmute(f);
    f(&from_glib_none(this), context_id, &String::from_glib_none(text))
}

unsafe extern "C" fn text_pushed_trampoline(this: *mut GtkStatusbar, context_id: c_uint, text: *mut c_char, f: gpointer) {
    callback_guard!();
    let f: &Box_<Fn(&Statusbar, u32, &str) + 'static> = transmute(f);
    f(&from_glib_none(this), context_id, &String::from_glib_none(text))
}
