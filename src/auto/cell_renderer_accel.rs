...
impl<O: IsA<CellRendererAccel> + IsA<glib::object::Object>> CellRendererAccelExt for O {
    ...
    fn get_property_accel_mode(&self) -> CellRendererAccelMode {
        unsafe {
            let mut value = Value::uninitialized();
            gobject_ffi::g_value_init(value.to_glib_none_mut().0, <CellRendererAccelMode as StaticType>::static_type().to_glib());
    ...
    fn get_property_accel_mods(&self) -> gdk::ModifierType {
        unsafe {
            let mut value = Value::uninitialized();
            gobject_ffi::g_value_init(value.to_glib_none_mut().0, <gdk::ModifierType as StaticType>::static_type().to_glib());
