...
impl<O: IsA<ComboBox> + IsA<Object>> ComboBoxExt for O {
    ...
    fn set_active_id<'a, P: Into<Option<&'a str>>>(&self, active_id: P) -> bool {
        ...
        let active_id = active_id.to_glib_none().0;
        unsafe {
            from_glib(ffi::gtk_combo_box_set_active_id(self.to_glib_none().0, active_id))
        }
    }
    ...
    fn set_model<'a, P: IsA<TreeModel> + 'a, Q: Into<Option<&'a P>>>(&self, model: Q) {
        ...
        unsafe {
            ffi::gtk_combo_box_set_model(self.to_glib_none().0, model);
        }
    }
...
where P: IsA<ComboBox> {
    callback_guard!();
    let f: &Box_<Fn(&P, &str) -> String + 'static> = transmute(f);
