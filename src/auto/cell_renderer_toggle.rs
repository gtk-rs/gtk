...
pub trait CellRendererToggleExt {
    ...
    fn connect_property_activatable_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_active_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_inconsistent_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_indicator_size_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_radio_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;
}
...
where P: IsA<CellRendererToggle> {
    ...
    f(&CellRendererToggle::from_glib_none(this).downcast_unchecked())
}
