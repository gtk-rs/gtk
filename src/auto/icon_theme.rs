...
impl IconTheme {
    ...
    pub fn get_for_screen(screen: &gdk::Screen) -> Option<IconTheme> {
        assert_initialized_main_thread!();
        unsafe {
            from_glib_none(gtk_sys::gtk_icon_theme_get_for_screen(screen.to_glib_none().0))
        }
    }
}
...
impl<O: IsA<IconTheme>> IconThemeExt for O {
    ...
    fn load_surface<P: IsA<gdk::Window>>(&self, icon_name: &str, size: i32, scale: i32, for_window: Option<&P>, flags: IconLookupFlags) -> Result<Option<cairo::Surface>, Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let ret = gtk_sys::gtk_icon_theme_load_surface(self.as_ref().to_glib_none().0, icon_name.to_glib_none().0, size, scale, for_window.map(|p| p.as_ref()).to_glib_none().0, flags.to_glib(), &mut error);
            if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) }
        }
    }
    ...
    fn set_screen(&self, screen: &gdk::Screen) {
        unsafe {
            gtk_sys::gtk_icon_theme_set_screen(self.as_ref().to_glib_none().0, screen.to_glib_none().0);
        }
    }
