...
impl<O: IsA<IMContext>> IMContextExt for O {
    ...
    fn filter_keypress(&self, event: &gdk::EventKey) -> bool {
        unsafe {
            from_glib(gtk_sys::gtk_im_context_filter_keypress(self.as_ref().to_glib_none().0, mut_override(event.to_glib_none().0)))
        }
    }
    ...
    fn set_client_window<P: IsA<gdk::Window>>(&self, window: Option<&P>) {
        unsafe {
            gtk_sys::gtk_im_context_set_client_window(self.as_ref().to_glib_none().0, window.map(|p| p.as_ref()).to_glib_none().0);
        }
    }
