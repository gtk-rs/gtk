...
impl AboutDialog {
    ...
    pub fn connect_activate_link<F: Fn(&AboutDialog, &str) -> bool + 'static>(&self, f: F) -> u64 {
        unsafe {
            let f: Box_<Box_<Fn(&AboutDialog, &str) -> bool + 'static>> = Box_::new(Box_::new(f));
...
unsafe extern "C" fn activate_link_trampoline(this: *mut GtkAboutDialog, uri: *mut c_char, f: gpointer) -> gboolean {
    callback_guard!();
    let f: &Box_<Fn(&AboutDialog, &str) -> bool + 'static> = transmute(f);
