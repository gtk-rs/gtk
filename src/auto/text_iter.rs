...
impl TextIter {
    ...
    pub fn begins_tag<P: IsA<TextTag>>(&self, tag: Option<&P>) -> bool {
        unsafe {
            from_glib(gtk_sys::gtk_text_iter_begins_tag(self.to_glib_none().0, tag.map(|p| p.as_ref()).to_glib_none().0))
        }
    }
    ...
    pub fn get_pixbuf(&self) -> Option<gdk_pixbuf::Pixbuf> {
        unsafe {
            from_glib_none(gtk_sys::gtk_text_iter_get_pixbuf(self.to_glib_none().0))
        }
    }
