...
impl ListStore {
    pub fn new(column_types: &[Type]) -> ListStore {
        ...
        unsafe {
            ...
            from_glib_full(
                gtk_sys::gtk_list_store_newv(column_types.len() as c_int,
...
impl<O: IsA<ListStore>> GtkListStoreExtManual for O {
    fn insert_with_values(
        ...
        unsafe {
            ...
            for (&column, value) in columns.iter().zip(values.iter()) {
                ...
                let type_ = from_glib(
                    gtk_sys::gtk_tree_model_get_column_type(self.as_ref().upcast_ref::<TreeModel>().to_glib_none().0, column as c_int));
                assert!(Value::type_transformable(value.to_value_type(), type_));
            }
            let mut iter = TreeIter::uninitialized();
            gtk_sys::gtk_list_store_insert_with_valuesv(self.as_ref().to_glib_none().0,
    ...
    fn reorder(&self, new_order: &[u32]) {
        unsafe {
            let count = gtk_sys::gtk_tree_model_iter_n_children(self.as_ref().upcast_ref::<TreeModel>().to_glib_none().0, ptr::null_mut());
            ...
            if safe_count && safe_values {
                gtk_sys::gtk_list_store_reorder(self.as_ref().to_glib_none().0,
    ...
    fn set(&self, iter: &TreeIter, columns: &[u32], values: &[&dyn ToValue]) {
        unsafe {
            ...
            gtk_sys::gtk_list_store_set_valuesv(self.as_ref().to_glib_none().0,
    ...
    fn set_value(&self, iter: &TreeIter, column: u32, value: &Value) {
        unsafe {
            let columns = gtk_sys::gtk_tree_model_get_n_columns(self.as_ref().upcast_ref::<TreeModel>().to_glib_none().0);
            assert!(column < columns as u32);
            let type_ = from_glib(
                gtk_sys::gtk_tree_model_get_column_type(self.as_ref().upcast_ref::<TreeModel>().to_glib_none().0, column as c_int));
            assert!(Value::type_transformable(value.type_(), type_));
            gtk_sys::gtk_list_store_set_value(self.as_ref().to_glib_none().0,
